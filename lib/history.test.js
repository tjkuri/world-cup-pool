...
import { buildHistorySeries, rankMovements, rankVolatility } from './history.js';
...
test('rankVolatility sums absolute rank changes across snapshots', () => {
  // A and B swap ranks every snapshot: A ranks [1,2,1], B ranks [2,1,2].
  const snaps = [
    { t: 't0', standings: [ { email_hash: 'a', name: 'A', total: 10 }, { email_hash: 'b', name: 'B', total: 5 } ] },
    { t: 't1', standings: [ { email_hash: 'a', name: 'A', total: 5 },  { email_hash: 'b', name: 'B', total: 10 } ] },
    { t: 't2', standings: [ { email_hash: 'a', name: 'A', total: 10 }, { email_hash: 'b', name: 'B', total: 5 } ] },
  ];
  const vol = rankVolatility({ snapshots: snaps });
  assert.equal(vol.find((x) => x.email_hash === 'a').volatility, 2); // |1-2|+|2-1|
  assert.equal(vol.find((x) => x.email_hash === 'b').volatility, 2);
});
