...
export function scoreBracket(bracket, knockout, results) {
  ...
  if (finalSlot) {
    ...
    if (actualFinal && pickFinal) {
      ...
      // Cap at 100: max two correct finalists.
...
// drilldown. Winner points by round (the final's winner is the champion at 80),
// plus the exact-score bonus. Finalist achievements are bracket-level and not
// attributed to a single match here. `round` is 'R32'|'R16'|'QF'|'SF'|'F';
...
export function scoreKnockoutMatch(round, pick, actual) {
  ...
  const exact = pick.home_score === actual.home_score && pick.away_score === actual.away_score;
