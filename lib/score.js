...
const POINTS_FIRST_PLACE = 20;
const POINTS_SECOND_PLACE = 10;
const POINTS_THIRD_PLACE = 5;
const POINTS_FOURTH_PLACE = 2;
const POINTS_PERFECT_ORDER = 10;
...
export function scoreSubmission(submission, fixtures, results) {
  ...
  for (const [groupLetter, predicted] of Object.entries(submission.group_standings)) {
    ...
    const fourth = predicted[3] === actual[3] ? POINTS_FOURTH_PLACE : 0;
    const perfect = arraysEqual(predicted, actual) ? POINTS_PERFECT_ORDER : 0;
    const subtotal = first + second + third + fourth + perfect;
    group_points[groupLetter] = { first, second, third, fourth, perfect, subtotal };
