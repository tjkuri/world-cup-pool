...
test('all 4 positions correct + perfect bonus = 47 pts', () => {
  const fixtures = singleGroupFixtures();
  ...
  const r = scoreSubmission(submission, fixtures, results);
  assert.equal(r.group_points.A.first, 20);
  assert.equal(r.group_points.A.second, 10);
  assert.equal(r.group_points.A.third, 5);
  assert.equal(r.group_points.A.fourth, 2);
  assert.equal(r.group_points.A.perfect, 10);
  assert.equal(r.group_points.A.subtotal, 47);
});

test('1st + 4th correct, 2nd/3rd swapped: 22 pts, no perfect bonus', () => {
  const fixtures = singleGroupFixtures();
  const allWin = {
    m01: { home_score: 1, away_score: 0 },
    m02: { home_score: 1, away_score: 0 },
  ...
  const submission = {
    matches: allWin,
    group_standings: { A: ['AAA','CCC','BBB','DDD'] } // 1st right, 2nd+3rd swapped, 4th right
  };
  const results = { matches: Object.fromEntries(Object.entries(allWin).map(([k, v]) => [k, { ...v, status: 'STATUS_FINAL' }])) };
  const r = scoreSubmission(submission, fixtures, results);
  assert.equal(r.group_points.A.first, 20);
  ...
  assert.equal(r.group_points.A.subtotal, 22);
});

test('total = match_total + group_total: max single group = 83 pts', () => {
  const fixtures = singleGroupFixtures();
  const allWin = {
    m01: { home_score: 1, away_score: 0 },
    m02: { home_score: 1, away_score: 0 },
  ...
  const results = { matches: Object.fromEntries(Object.entries(allWin).map(([k, v]) => [k, { ...v, status: 'STATUS_FINAL' }])) };
  const r = scoreSubmission(submission, fixtures, results);
  // All 6 matches scored exactly: 6 * 6 = 36. Group total: 47. Total: 83.
  assert.equal(r.match_total, 36);
  assert.equal(r.group_total, 47);
