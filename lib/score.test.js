...
test('scoreBracket awards round-winner points per correct slot advancer', () => {
  ...
  assert.equal(s.champion_points, 64);          // F advancer BRA, picked BRA
...
test('scoreKnockoutMatch: winner + exact, champion on final, pending when not final', () => {
  ...
  // final winner = champion 64 + exact-final 5 → 69
  assert.equal(scoreKnockoutMatch('F', { home_score: 2, away_score: 1, advances: 'BRA' }, actual).points, 69);
