...
- **Scoring rebalance (2026-06-24):** finalist points (50 each / 100 max) removed entirely — in a connected bracket a correct SF-winner pick already *is* a correct finalist, so it double-rewarded the same pick — and champion dropped 80→64 for a clean doubling ladder (4→8→16→32→64). KO max ~531→~415, total ~1,383→~1,267, phase split 62/38→~67/33. `scoreBracket` no longer returns `finalist_points`.
